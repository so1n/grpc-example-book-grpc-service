...
class ManagerService(manager_service.BookManagerServicer):
    ...
    @conn_proxy()
    def create_book(
        self, request: manager_message.CreateBookRequest, context: grpc.ServicerContext
    ) -> Empty:
    ...
    @conn_proxy()
    def delete_book(
        self, request: manager_message.DeleteBookRequest, context: grpc.ServicerContext
    ) -> Empty:
    ...
    @conn_proxy()
    def get_book(
        self, request: manager_message.GetBookRequest, context: grpc.ServicerContext
    ) -> manager_message.GetBookResult:
        return manager_message.GetBookResult(
            **proto_load(manager_dal.get_book(isbn=request.isbn))
        )

    @conn_proxy()
    def get_book_list(
        self, request: manager_message.GetBookListRequest, context: grpc.ServicerContext
    ) -> manager_message.GetBookListResult:
        return manager_message.GetBookListResult(
            result=proto_load(
                manager_dal.get_book_list(
